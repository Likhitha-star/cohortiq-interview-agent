...
/** Scores shown in the completed interview feedback. */
...
export interface InterviewState {
  ...
  /** Exact question texts already used. */
  ...
  /** Ordered list of curriculum days to prioritize. */
  topicPlan: number[];

  /** Index into topicPlan for the next primary question day. */
  topicPlanIndex: number;

  /** Whether the next question should be a follow-up. */
  pendingFollowUpDay: number | null;

  /** Objectives/tools already referenced for each day. */
...
/**
 * Score a mission for topic selection.
 * Higher score means the topic is more useful for the interview.
 */
...
/** Select at least MIN_CURRICULUM_DAYS distinct days. */
...
export function buildFollowUpQuestion(
  ...
  const unmentionedObjectives = day.objectives.filter(
    (objective) => {
  ...
  const remainingPrimary = buildQuestionCandidates(
    day,
    coveredPromptsByDay,
  );
...
function requirementsMet(state: InterviewState): boolean {
  const answeredDays = uniqueDays(
    state.answers.map((entry) => entry.day),
  );
...
function nextPrimaryDay(state: InterviewState): number {
  ...
  const day =
    state.topicPlan[
      state.topicPlanIndex % state.topicPlan.length
    ] ?? state.topicPlan[0];
...
function pickNextQuestion(
  ...
  if (state.pendingFollowUpDay !== null) {
    const day = getCurriculumDay(
      state.pendingFollowUpDay,
    );
  ...
  const primaryDay =
    getCurriculumDay(primaryDayNumber);
...
function shouldOfferFollowUp(
  ...
  if (
    daysStillNeeded > 0 &&
    questionsRemaining <= daysStillNeeded
  ) {
    return false;
  }
...
function advanceTopicPlanIndex(
  state: InterviewState,
): number {
  if (state.topicPlan.length === 0) {
    return 0;
  ...
  return (
    (state.topicPlanIndex + 1) %
    state.topicPlan.length
  );
}

/**
 * Produce deterministic fallback feedback.
 *
 * AI-powered scoring is handled separately by the
 * Claude/Groq service. These values prevent the engine
 * from breaking if deterministic feedback is used.
 */
export function generateFeedback(
  ...
  for (const mission of candidate.missions) {
    ...
    if (isMissionSkipped(mission)) {
      gaps.push(
        `Skipped Day ${mission.day}: ${mission.title}.`,
      );
    ...
    if (
      ...
      gaps.push(
        `Did not pass Day ${mission.day}: ${mission.title}.`,
      );
  ...
  const detailedAnswers = answers.filter(
    (entry) =>
      entry.answer.trim().length >= 100,
  );

  if (
    detailedAnswers.length >=
    Math.min(4, answers.length)
  ) {
    strengths.push(
  ...
  const shortAnswers = answers.filter(
    (entry) =>
      entry.answer.trim().length < 50,
  );

  if (shortAnswers.length >= 3) {
    gaps.push(
      "Several interview answers lacked sufficient technical depth.",
    );
  ...
  const interviewedDays = uniqueDays(
    daysCovered,
  );

  const summary =
    `Interview with ${candidate.member.name} (${candidate.member.jobRole}) covered ${interviewedDays.length} curriculum days across ${answers.length} questions. Overall cohort progress: ${candidate.signals.missionsCompleted} missions completed with ${candidate.signals.missionsFirstTry} first-try passes.`;

  /*
   * Basic deterministic scores.
   *
   * These are fallback scores only.
   * The AI feedback service can replace them
   * with evidence-based scores.
   */
  ...
  const detailedRatio =
    detailedAnswers.length / totalAnswers;

  const shortRatio =
    shortAnswers.length / totalAnswers;
  ...
  const implementationDepth = Math.round(
    Math.max(
      0,
      Math.min(
        100,
  ...
  const technicalKnowledge = Math.round(
    Math.max(
      0,
      Math.min(
        100,
        50 +
          (interviewedDays.length /
            Math.max(
              MIN_CURRICULUM_DAYS,
              1,
            )) *
...
/**
 * Create initial interview state for a candidate.
 */
export function createInterviewState(
  ...
  const scoredTopics =
    buildTopicPlan(candidate);

  const topicPlan =
    selectTopicDays(scoredTopics);
...
export function startInterview(
  ...
  const next =
    pickNextQuestion(state);

  if (!next) {
    throw new Error(
      "Unable to generate the first interview question.",
    );
  }
...
export function submitAnswer(
  ...
  if (!trimmedAnswer) {
    return {
      reply:
        "Please provide an answer before we continue.",
      done: false,
      state,
    };
  }
